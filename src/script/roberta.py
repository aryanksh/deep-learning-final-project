...
import torch
import sklearn  # kept for consistency with the original notebook
import numpy as np
import pandas as pd
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_roberta-lr0.00005")
...
def roberta_tokenization(example):
...
ds_train_tokenized_roberta.set_format(
    type="torch", columns=["input_ids", "attention_mask", "label"]
)
ds_test_tokenized_roberta.set_format(
    type="torch", columns=["input_ids", "attention_mask", "label"]
)
ds_val_tokenized_roberta.set_format(
    type="torch", columns=["input_ids", "attention_mask", "label"]
)

print("Tokenized train format:", ds_train_tokenized_roberta.format)
